...
while not game_over:
    ...
    for event in pygame.event.get():
        ...
        if turn == PLAYER:
            col1, minimax_score1 = alphabeta(board, 2, -math.inf, math.inf, True)

            if is_valid_location(board, col1):
                row = get_next_open_row(board, col1)
                drop_piece(board, row, col1, PLAYER_PIECE)
    ...
    if turn == AI and not game_over:
        ...
        if is_valid_location(board, col):
            ...
            if winning_move(board, AI_PIECE):
                label = myfont.render("Player 2 wins!!", 5, YELLOW)
                screen.blit(label, (40, 10))
                game_over = True
